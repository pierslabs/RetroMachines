...
class juego{
	...
	render(){
		...
		//  contador de vueltas
            ...
            if ( this.vueltas === true && this.car.direction == "top" | this.car.direction == "top_left" | this.car.direction == "top_right" ){
                this.car.numLap++;
             }
		 ...
		 // Render
            ...
            // Carga mapeado
            this.mapcnx2.drawImage(this.map_circuito,this.car.x, this.car.y, 4592, 5270 );
			// Carga coche
            this.pinta_coche();
            // this.phantom();
	...
	mueve_coche(multi){
        ...
        var cardir = this.car.direction;
            ...
            if(cardir == "top"){		this.car.y += (6 * multi) ; }
            if(cardir == "top_right"){	this.car.x -= (5 * multi); this.car.y += (5 * multi); }
            if(cardir == "right"){		this.car.x -= (7 * multi); }
            if(cardir == "down_right"){	this.car.x -= (5 * multi); this.car.y -= (5 * multi); }
            if(cardir == "down"){		this.car.y -= (6 * multi); }
            if(cardir == "down_left"){	this.car.x += (5 * multi); this.car.y -= (5 * multi); }
            if(cardir == "top_left"){	this.car.x += (5 * multi); this.car.y += (5 * multi); }
            if(cardir == "left"){	    this.car.x += (7 * multi); }
    ...
    siguintePaso(){
		...
		if(dir == "top")        { var pixel = gmap2.getImageData(495, 330, 20, 1);}
		if(dir == "top_right")  { var pixel = gmap2.getImageData(519, 330, 1, 12);}
		if(dir == "right")      { var pixel = gmap2.getImageData(520, 336, 1, 20);}
		if(dir == "down_right") { var pixel = gmap2.getImageData(522, 348, 1, 10);}
		if(dir == "down")       { var pixel = gmap2.getImageData(490, 360, 25, 1);}
		if(dir == "down_left")  { var pixel = gmap2.getImageData(485, 365, 15, 1);}
		if(dir == "left")       { var pixel = gmap2.getImageData(485, 375, 1, 10);}
		if(dir == "top_left")   { var pixel = gmap2.getImageData(485, 325, 1, 15);}
		
		// RGBA 4
			// 25x2 = 50 pixeles x 4 canales = 200
			var paso = "ok";
			for (let i = 0; i < pixel.data.length; i = i+4) {
				if(( pixel.data[i] == game.colors.background.r && pixel.data[i+1] == game.colors.background.g && pixel.data[i+2] == game.colors.background.b)){
					 // Fondo todo OK
					 paso = "ok";
				}else if(( pixel.data[i] == game.colors.oil.r && pixel.data[i+1] == game.colors.oil.g && pixel.data[i+2] == game.colors.oil.b)){
					 // OIL
					 paso = "oil";
				}else if(( pixel.data[i] == game.colors.glue.r && pixel.data[i+1] == game.colors.glue.g && pixel.data[i+2] == game.colors.glue.b)){
					// GLUE
					paso = "glue";
				}else if(( pixel.data[i] == game.colors.crash.r && pixel.data[i+1] == game.colors.crash.g && pixel.data[i+2] == game.colors.crash.b)){
					// CRASH
					paso = "crash";
                }
                return paso;
    ...
    countLap(){
       ...
       if(( gmap2.data[0] == game.colors.lap.r && gmap2.data[1] == game.colors.lap.g && gmap2.data[2] == game.colors.lap.b)){
            return true;
       } 
    }
...
class keyboard{
	...
	save_position(){
		if(this.letters.ArrowUp && !this.letters.ArrowDown && this.letters.ArrowRight && !this.letters.ArrowLeft){
			game.car.direction = "top_right";
		}else if(this.letters.ArrowUp && !this.letters.ArrowDown && !this.letters.ArrowRight && this.letters.ArrowLeft){
